...
{
        pr_info("calling tx intrupt handler function\n");
...
{
        pr_info("calling tx intrupt function\n");
...
{
        pr_info("calling rx intrupt function\n");
	struct cpsw_common *cpsw = dev_id;
...
{
        pr_info("calling tx_mq_poll function\n");
...
{
        pr_info("calling tx_poll function\n");
	struct cpsw_common *cpsw = napi_to_cpsw(napi_tx);
	int num_tx;
...
{
pr_info("Calling rx_mq_poll fucntion \n");
...
{       pr_info("calling rx_poll fucntion\n");
	struct cpsw_common *cpsw = napi_to_cpsw(napi_rx);
	int num_rx;
...
{
    /* Common CPSW structure (shared hardware data) */
    struct cpsw_common *cpsw = priv->cpsw;

    /* Metadata stored inside RX page (used for XDP / RX completion) */
    struct cpsw_meta_xdp *xmeta;

    /* Per-RX-channel page pool */
    struct page_pool *pool;

    /* RX buffer (full memory page) */
    struct page *page;

    /* Number of RX buffers needed for this channel */
    int ch_buf_num;

    /* Channel index, loop index, return code */
    int ch, i, ret;

    /* DMA address where hardware will write packet */
    dma_addr_t dma;

    /* Iterate over all RX DMA channels */
    for (ch = 0; ch < cpsw->rx_ch_num; ch++) {

        /* Get page pool associated with this RX channel */
        pool = cpsw->page_pool[ch];

        /* Ask CPDMA how many RX buffers are currently missing */
        ch_buf_num = cpdma_chan_get_rx_buf_num(cpsw->rxv[ch].ch);

        /* Allocate and submit required number of RX buffers */
        for (i = 0; i < ch_buf_num; i++) {

            /* Allocate one RX page from page pool */
            ...
            /* Store CPSW-specific metadata inside the page */
            xmeta = page_address(page) + CPSW_XMETA_OFFSET;
            xmeta->ndev = priv->ndev; /* which netdev */
            xmeta->ch = ch;           /* which RX channel */

            /* Get DMA address for the page and add headroom */
            dma = page_pool_get_dma_addr(page) + CPSW_HEADROOM;

            /* Submit RX buffer to CPDMA RX channel */
            ret = cpdma_chan_idle_submit_mapped(
                        cpsw->rxv[ch].ch,
                        page,                /* buffer cookie */
                        dma,                 /* DMA address */
                        cpsw->rx_packet_max, /* max packet size */
                        0);                  /* flags */

            /* If submission fails, recycle page and abort */
            if (ret < 0) {
                cpsw_err(priv, ifup,
        ...
        /* Informational log: RX channel fully populated */
        cpsw_info(priv, ifup,
                  "ch %d rx, submitted %d descriptors\n",
                  ch, ch_buf_num);
    }

    /* All RX channels successfully filled */
    return 0;
}
