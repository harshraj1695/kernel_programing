...
static int cpsw_ndo_stop(struct net_device *ndev)
{
    ...
    /* ---- Stop Per-Port (Slave) Logic ---- */

    /* Stop CPSW slave ports (PHY, MAC port, link handling) */
    for_each_slave(priv, cpsw_slave_stop, cpsw);

    /* ---- Resource Reconfiguration ---- */

    /* Restore resource split if CPSW was in dual-EMAC mode */
    if (cpsw_need_resplit(cpsw))
        cpsw_split_res(cpsw);

    /* Decrement number of active netdev users */
    cpsw->usage_count--;

    /* Release runtime PM reference (allow hardware to power down) */
    pm_runtime_put_sync(cpsw->dev);
...
{
        pr_info("Caling ndo_start_xmit fucntion\n");
	struct cpsw_priv *priv = netdev_priv(ndev);
	struct cpsw_common *cpsw = priv->cpsw;
...
{
    /* Debug: probe entry */
    pr_info("In the cpsw_probe function\n");

    /* Generic device pointer from platform bus */
    struct device *dev = &pdev->dev;

    /* Clock handle (used to derive bus frequency) */
    struct clk *clk;

    /* Platform data parsed from Device Tree */
    struct cpsw_platform_data *data;

    /* Linux network device (eth0) */
    struct net_device *ndev;

    /* Per-netdev private data */
    struct cpsw_priv *priv;

    /* CPSW subsystem register base */
    void __iomem *ss_regs;

    /* Memory resource for CPSW registers */
    struct resource *ss_res;

    /* Optional GPIOs to select CPSW mode */
    struct gpio_descs *mode;

    /* SoC matching info (for quirks) */
    const struct soc_device_attribute *soc;

    /* Common CPSW structure shared across ports */
    struct cpsw_common *cpsw;

    int ret = 0, ch;
    int irq;

    /* Allocate CPSW common structure (zeroed) */
    cpsw = devm_kzalloc(dev, sizeof(struct cpsw_common), GFP_KERNEL);
    if (!cpsw)
        return -ENOMEM;

    /* Store cpsw pointer in platform device */
    platform_set_drvdata(pdev, cpsw);

    /* Save global slave index (used for dual-emac setups) */
    cpsw_slave_index = cpsw_slave_index_priv;

    cpsw->dev = dev;

    /* Optional GPIOs to configure CPSW mode (RGMII/MII/etc) */
    ...
    /* Convert clock rate to MHz (used by CPDMA shaper, etc.) */
    cpsw->bus_freq_mhz = clk_get_rate(clk) / 1000000;

    /* Get CPSW register memory region */
    ss_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);

    /* Map CPSW subsystem registers */
    ...
    /* Map CPSW wrapper registers (interrupt, pacing, misc control) */
    ...
    /* Misc interrupt (CPTS, errors, etc.) */
    irq = platform_get_irq(pdev, 3);
    if (irq <= 0)
        return irq;
    cpsw->misc_irq = irq;

    /* Enable runtime PM for CPSW */
    pm_runtime_enable(dev);

    /* Power up CPSW so registers can be accessed */
    ...
    /* Parse Device Tree: ports, PHYs, MAC, VLAN, etc. */
    pr_info("Calling probe dt function\n");
    ret = cpsw_probe_dt(&cpsw->data, pdev);
    if (ret)
        goto clean_dt_ret;

    /* Match SoC for known hardware quirks */
    pr_info("calling soc device matching function\n");
    ...
    /* Allocate CPSW slave structures (ports) */
    ...
    /* Descriptor pool size for CPDMA */
    cpsw->descs_pool_size = descs_pool_size;

    /* Initialize CPSW core:
     * - ALE
     * - CPDMA controller
     * - statistics
     */
    ...
    /* Select DMA channel number (depends on IRQ quirk) */
    ch = cpsw->quirk_irq ? 0 : 7;

    pr_info("Passing cpsw_rx or tx according to interrupt\n");

    /* Create TX DMA channel and attach TX completion handler */
    cpsw->txv[0].ch = cpdma_chan_create(cpsw->dma, ch,
                                        cpsw_tx_handler, 0);
    ...
    /* Create RX DMA channel and attach RX completion handler */
    cpsw->rxv[0].ch = cpdma_chan_create(cpsw->dma, 0,
                                        cpsw_rx_handler, 1);
    ...
    /* Split CPDMA resources between channels */
    cpsw_split_res(cpsw);

    /* Allocate Linux net_device with TX/RX queues */
    ndev = devm_alloc_etherdev_mqs(dev,
    ...
    /* Set MAC address (DT or random) */
    if (is_valid_ether_addr(data->slave_data[0].mac_addr)) {
        memcpy(priv->mac_addr,
               data->slave_data[0].mac_addr,
               ETH_ALEN);
    ...
    /* Enable VLAN hardware features */
    ndev->features |= NETIF_F_HW_VLAN_CTAG_FILTER |
                      NETIF_F_HW_VLAN_CTAG_RX;

    /* Attach netdev operations and ethtool operations */
    ndev->netdev_ops = &cpsw_netdev_ops;
    ndev->ethtool_ops = &cpsw_ethtool_ops;

    /* Register RX and TX NAPI contexts */
    ...
    /* Register net_device with kernel */
    ...
    /* Setup second netdev if dual-emac */
    if (cpsw->data.dual_emac) {
    ...
    /* Request RX interrupt (top half) */
    ret = devm_request_irq(dev, cpsw->irqs_table[0],
                           cpsw_rx_interrupt,
                           0, dev_name(dev), cpsw);
    if (ret < 0) {
    ...
    /* Request TX interrupt */
    ret = devm_request_irq(dev, cpsw->irqs_table[1],
                           cpsw_tx_interrupt,
    ...
    /* Request MISC interrupt (CPTS, errors) */
    if (cpsw->cpts) {
        ret = devm_request_irq(&pdev->dev, cpsw->misc_irq,
                               cpsw_misc_interrupt,
                               0, dev_name(&pdev->dev), cpsw);
        if (ret < 0) {
