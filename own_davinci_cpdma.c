...
int cpdma_ctlr_start(struct cpdma_ctlr *ctlr)
{
    ...
    /* Protect controller state & registers from concurrent access
     * IRQs must be disabled because CPDMA can be touched from IRQ context
     */
    spin_lock_irqsave(&ctlr->lock, flags);

    /* CPDMA can only be started from IDLE state */
    if (ctlr->state != CPDMA_STATE_IDLE) {
        spin_unlock_irqrestore(&ctlr->lock, flags);
        return -EBUSY;
    }

    /* ---- Optional CPDMA Soft Reset ---- */
    if (ctlr->params.has_soft_reset) {
        unsigned timeout = 10 * 100;

        /* Trigger soft reset of CPDMA engine */
        dma_reg_write(ctlr, CPDMA_SOFTRESET, 1);

        /* Poll until reset bit clears (hardware clears it) */
        while (timeout) {
        ...
        /* Warn if hardware never completed reset */
        WARN_ON(!timeout);
    }

    /* ---- Clear DMA Head and Completion Pointers ---- */
    ...
    /* ---- Disable and clear all CPDMA interrupts ---- */
    ...
    for (i = 0; i < ARRAY_SIZE(ctlr->channels); i++) {
        chan = ctlr->channels[i];
        if (chan) {
            /* Program rate limiting / shaping for channel */
            cpdma_chan_set_chan_shaper(chan);

            /* Enable the DMA channel */
            cpdma_chan_on(chan);

            /* Enable priority mode if at least one TX channel
             * is NOT rate limited
             */
    ...
    /* ---- Global CPDMA Control Configuration ---- */

    /* Configure TX priority scheduling mode */
    _cpdma_control_set(ctlr, CPDMA_TX_PRIO_FIXED, prio_mode);

    /* Configure RX buffer offset (no offset used here) */
...
{
	/* Each CPDMA channel register block is 4 bytes apart */
	...
	/*
	 * Convert logical RX/TX channel number into
	 * hardware channel numbering.
	 *
	 * Hardware uses a linear numbering for all channels,
	 * while the driver may pass RX/TX channels separately.
	 */
	chan_num = rx_type ? rx_chan_num(chan_num) : tx_chan_num(chan_num);

	/*
	 * Validate that the requested channel exists
	 * in this CPDMA controller.
	 */
	if (__chan_linear(chan_num) >= ctlr->num_chan)
		return ERR_PTR(-EINVAL);

	/*
	 * Allocate and zero-initialize channel structure.
	 * devm_* ensures automatic cleanup on device removal.
	 */
	...
	/*
	 * Protect controller-wide channel list.
	 * Channel creation may race with IRQ/NAPI paths.
	 */
	spin_lock_irqsave(&ctlr->lock, flags);

	/*
	 * Ensure this hardware channel is not already in use.
	 */
	...
	chan->ctlr	= ctlr;              /* Parent CPDMA controller */
	chan->state	= CPDMA_STATE_IDLE;  /* Channel starts idle */
	chan->chan_num	= chan_num;          /* Hardware channel number */
	chan->handler	= handler;           /* RX/TX completion callback */
	chan->rate	= 0;                 /* QoS (if supported) */
	chan->weight	= 0;                 /* Scheduling weight */

	/*
	 * RX and TX channels have different registers,
	 * interrupts, and DMA directions.
	 */
	if (is_rx_chan(chan)) {
		/* RX Host Descriptor Pointer register */
		chan->hdp	= ctlr->params.rxhdp + offset;

		/* RX Completion Pointer register */
		chan->cp	= ctlr->params.rxcp + offset;

		/* RX Free Buffer Count register */
		chan->rxfree	= ctlr->params.rxfree + offset;

		/* RX interrupt mask control registers */
		...
		/* TX Host Descriptor Pointer register */
		chan->hdp	= ctlr->params.txhdp + offset;

		/* TX Completion Pointer register */
		chan->cp	= ctlr->params.txcp + offset;

		/* TX interrupt mask control registers */
		chan->int_set	= CPDMA_TXINTMASKSET;
		chan->int_clear	= CPDMA_TXINTMASKCLEAR;

		/* TX teardown register */
		chan->td	= CPDMA_TXTEARDOWN;

		/* DMA direction: memory → device */
		chan->dir	= DMA_TO_DEVICE;
	}

	/*
	 * Interrupt mask bit corresponding to this channel.
	 * Used to enable/disable and acknowledge interrupts.
	 */
	chan->mask = BIT(chan_linear(chan));

	/* Initialize per-channel lock for descriptor operations */
	spin_lock_init(&chan->lock);

	/*
	 * Register this channel with the controller
	 * so IRQ/NAPI paths can locate it.
	 */
	ctlr->channels[chan_num] = chan;
	ctlr->chan_num++;

	/*
	 * Repartition descriptor pool across all active channels
	...
	/* Channel is fully initialized and ready */
	return chan;
}
...
{
	/* Debug: called when a DMA descriptor is fully completed */
	pr_info("calling __cpdma_chan_free function\n");

	/* Parent CPDMA controller */
	struct cpdma_ctlr		*ctlr = chan->ctlr;

	/* Descriptor pool for allocation/free */
	struct cpdma_desc_pool		*pool = ctlr->pool;

	/* DMA (bus) address of the data buffer */
	dma_addr_t			buff_dma;

	/* Original buffer length used for DMA mapping */
	int				origlen;

	/* Opaque software token (skb, page, etc.) */
	uintptr_t			token;

	/*
	 * Retrieve software token stored during descriptor submission.
	 * This identifies the buffer to the upper RX/TX handler.
	 */
	token = desc_read(desc, sw_token);

	/*
	 * Retrieve original length of the DMA buffer.
	 * Must be used for DMA unmap/sync, not the actual output length.
	 */
	origlen = desc_read(desc, sw_len);

	/*
	 * Retrieve DMA address of the data buffer.
	 * This is the bus address used by the DMA engine.
	 */
	buff_dma = desc_read(desc, sw_buffer);

	/*
	 * Handle DMA memory cleanup depending on mapping type.
	 *
	 * If CPDMA_DMA_EXT_MAP is set, the buffer uses a persistent
	 * DMA mapping (e.g., RX page pool), so only cache sync is needed.
	 *
	 * Otherwise, the buffer was mapped for a single transaction
	 * and must be fully unmapped.
	 */
	if (origlen & CPDMA_DMA_EXT_MAP) {
		/* Remove EXT_MAP flag to get actual buffer length */
		origlen &= ~CPDMA_DMA_EXT_MAP;

		/* Make DMA-written data visible to the CPU */
		dma_sync_single_for_cpu(ctlr->dev, buff_dma, origlen,
					chan->dir);
	} else {
		/* Tear down DMA mapping for one-shot buffers */
		dma_unmap_single(ctlr->dev, buff_dma, origlen, chan->dir);
	}

	/*
	 * Return descriptor to the free pool so it can be reused
	 * for future DMA operations.
	 */
	cpdma_desc_free(pool, desc, 1);

	/*
	 * Notify upper layer of completion.
	 *
	 * RX: build skb/page and pass packet to network stack
	 * TX: free skb and update transmit statistics
	 */
...
{
	/* Debug log: called when processing one DMA descriptor */
	pr_debug("Calling __cpdma_chan_process function\n");
	...
	/* Descriptor status and output length */
	int				status, outlen;

	/* Status passed to upper-layer callback */
	int				cb_status = 0;

	/* Descriptor pool used for phys<->virt conversion */
	struct cpdma_desc_pool		*pool = ctlr->pool;
	...
	/*
	 * Protect channel descriptor list and hardware pointers.
	 * DMA completion and NAPI may run concurrently.
	 */
	spin_lock_irqsave(&chan->lock, flags);

	/*
	 * Get the first descriptor in the channel queue.
	 * 'head' points to the oldest outstanding descriptor.
	 */
	desc = chan->head;
	if (!desc) {
		/* No descriptor available to process */
		chan->stats.empty_dequeue++;
		status = -ENOENT;
		goto unlock_ret;
	}

	/* Get physical address of the descriptor */
	desc_dma = desc_phys(pool, desc);

	/*
	 * Read descriptor status word written by hardware.
	 * hw_mode contains ownership, length, EOQ, TD flags, etc.
	 */
	status	= desc_read(desc, hw_mode);

	/* Extract received/transmitted packet length (11-bit field) */
	outlen	= status & 0x7ff;

	/*
	 * If hardware still owns the descriptor,
	 * DMA has not completed yet.
	 */
	...
	/*
	 * If hardware already stripped CRC, adjust packet length.
	 */
	if (status & CPDMA_DESC_PASS_CRC)
		outlen -= CPDMA_DESC_CRC_LEN;

	/*
	 * Keep only meaningful status bits to pass upward:
	 *  - EOQ: end of queue
	 *  - TD_COMPLETE: teardown finished
	 *  - PORT info
	 *  - VLAN encapsulation
	 */
	...
	/*
	 * Advance software head to next descriptor
	 * using hardware-provided next pointer.
	 */
	chan->head = desc_from_phys(pool,
				    desc_read(desc, hw_next));

	/*
	 * Write completion pointer (CP register)
	 * to notify hardware that descriptor is reclaimed.
	 */
	chan_write(chan, cp, desc_dma);

	/* One less outstanding descriptor */
	chan->count--;

	/* Successfully dequeued one descriptor */
	chan->stats.good_dequeue++;

	/*
	 * EOQ handling:
	 * If end-of-queue reached and descriptors remain,
	 * restart DMA by writing HDP again.
	 */
	if ((status & CPDMA_DESC_EOQ) && chan->head) {
		chan->stats.requeue++;
		chan_write(chan, hdp,
			   desc_phys(pool, chan->head));
	}

	/* Release channel lock before invoking callbacks */
	spin_unlock_irqrestore(&chan->lock, flags);

	/*
	 * Teardown completion is signaled via TD_COMPLETE.
	 * Use special error code to notify upper layer.
	 */
	...
	/*
	 * Free descriptor and notify upper layer:
	 *  - RX: build skb and pass to stack
	 *  - TX: free skb and DMA mapping
	 */
	__cpdma_chan_free(chan, desc, outlen, cb_status);

	/* Return status to caller (used by quota logic) */
	return status;

unlock_ret:
	/* Unlock channel and return error/busy state */
	spin_unlock_irqrestore(&chan->lock, flags);
	return status;
}
...
int cpdma_chan_process(struct cpdma_chan *chan, int quota)
{
	...
	/*
	 * Channel must be active before processing descriptors.
	 * Processing an idle or tearing-down channel would corrupt state.
	 */
	if (chan->state != CPDMA_STATE_ACTIVE)
		return -EINVAL;

	/*
	 * Process completed descriptors up to the NAPI quota.
	 * Each iteration handles exactly one descriptor.
	 */
	while (used < quota) {

		/*
		 * Process a single completed descriptor.
		 * This checks DMA ownership, updates completion pointer,
		 * and invokes the RX/TX handler as needed.
		 */
		ret = __cpdma_chan_process(chan);

		/*
		 * Negative return means:
		 *  - no completed descriptor available, or
		 *  - channel is busy or in an invalid state.
		 * Stop processing further.
		 */
		if (ret < 0)
			break;

		/* One descriptor successfully processed */
		used++;
	}

	/*
	 * Return number of descriptors processed.
	 * This value is used by NAPI to manage polling budget
	 * and decide whether to re-enable interrupts.
	 */
	return used;
}
