...
static int my_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
  ...
  config1 = ioread8(ioaddr + 0x52);
  printk(KERN_INFO "RTL8139: Config1 initial value: 0x%02x\n", config1);

  // 2. UNLOCK the safety (using offset 0x50!)
  // 0xC0 sets bits 7 and 6 to '11' (Configuration Register Write Enable)
  iowrite8(0xC0, ioaddr + 0x50);
  printk(KERN_INFO "RTL8139: Keypad (0x50) set to Unlock mode\n");

  // 3. TRY TO CHANGE IT
  // Let's try to set Bit 0 (PMEN - Power Management Enable)
  iowrite8(config1 | 0x01, ioaddr + 0x52);

  // 4. VERIFY the change
  config1 = ioread8(ioaddr + 0x52);
  printk(KERN_INFO "RTL8139: Config1 after trying to change: 0x%02x\n",
         config1);

  // 5. LOCK the safety again
  iowrite8(0x00, ioaddr + 0x50);
  printk(KERN_INFO "RTL8139: Keypad (0x50) set back to Lock mode\n");
  return 0;
}
